({
  displayFieldData: function(cmp) {
    ...
    if (name != null && name != undefined && name != "") {
      var newSelectedUserObject = {
        ...
        CaseNumber: cmp.get("v.selRecName")
      };
      ...
      console.log("lstSelectedUsers>>> ", cmp.get("v.lstSelectedUsers"));
    }
  },

  handlerClick: function(cmp, event) {},
  ...
  doSearch: function(cmp) {
    ...
    action.setParams({
      ...
      sObjectName: cmp.get("v.sObjectAPIName")
    });
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      if (listOfselectedUsers && matches[i].SObjectId) {
        for (var j in listOfselectedUsers) {
          if (
            listOfselectedUsers[j].id &&
            listOfselectedUsers[j].id == matches[i].SObjectId
          ) {
      ...
      newComponents.push([
        ...
        {
          iconName:
            cmp.get("v.sObjectAPIName") == "User"
              ? "standard:user"
              : "custom:custom24", //listIconSVGPath,
          size: "small",
  ...
  handleSelection: function(cmp, event) {
    ...
    var updateLookupEvt = cmp.getEvent("MatchingRuleEvent");
  ...
  clearSelection: function(cmp, event) {
    ...
    if (listOfselectedUsers && objectId) {
      for (var i in listOfselectedUsers) {
        if (
          listOfselectedUsers[i].id &&
          listOfselectedUsers[i].id != objectId
        ) {
