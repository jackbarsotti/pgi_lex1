      ...
      updateCase({
        ...
        if (res) {
          ...
          if (this.caseCommentValue !== null && this.caseCommentValue !== '' && this.caseCommentValue != undefined) {
            insertCaseComment({
                caseCommentBody: this.caseCommentValue,
  ...
  objectRecordUi({
    ...
    if (data) {
      ...
      for (var key in layoutSec) {
            ...
            if (this.isNew && items[j].editableForNew) {
              ...
              for (var k in getfieldApi) {
                ...
                if (apiNameforMap !== null) {
                  ...
                  this.lowertoOriginalApi.push({
                    lower: apiNameforMap.toLowerCase(),
                    apiName: apiNameforMap
                  });
                  fieldAPIList.push({
                    apiName: getfieldApi[k].apiName,
                    editableForNew: items[j].editableForNew,
                    required: reqFields,
                    lower: apiNameforMap.toLowerCase()
                  });
            ...
            //edit functionality
              ...
              for (var k in getfieldApi) {
                ...
                if (apiNameforMap !== null) {
                  ...
                  fieldAPIList.push({
                    apiName: getfieldApi[k].apiName,
                    editableForNew: items[j].editableForUpdate,
                    required: reqFields,
                    lower: apiNameforMap.toLowerCase()
                  });
        ...
        this.layoutsection.push({
          sectionHeader: layoutSec[key].heading,
          sectionDetails: fieldAPIList
        });
      }
    ...
    if (this.isNew == true) {
      ...
      createCase({
        ...
        .then(result => {
          ...
          console.log('conn Result', this.recordId);
        })
        .catch(error => {
          //console.log('Error', error);
        });
      ...
      this.modes = 'Edit';
      console.log('connresult1', this.recordId);
      getRecordTypeDetail({
          recordId: this.recordId
        })
        .then(result => {
    ...
    getQuickCase()
      .then(result => {
        if (result !== null && result !== undefined) {
          ...
          this.quickCaseListOption = [{
            label: '--None--',
            value: '--None--'
          }];
          result.forEach(ele => {
            this.quickCaseListOption.push({
              label: ele.Name__c,
              value: ele.Name
            })
          })
        ...
        this.caseTemplateRecord.forEach(ele => {
          if (ele.Name === this.selectedCaseTemplate && ele.Default_Values__r !== null && ele.Default_Values__r !== undefined) {
            ...
            for (var key in defaultValueofTemplate) {
              defaultValRecordFields.push({
                apiName: defaultValueofTemplate[key].Field_API_Name__c,
                value: defaultValueofTemplate[key].Value__c
              });
            }
          }
        })
        ...
        defaultValRecordFields.forEach(ele => {
          for (var i in upperToLowerCaseApi) {
            ...
            if (ele.apiName === upperToLowerCaseApi[i].lower) {
              defaultTemplateValues.push({
                apiName: upperToLowerCaseApi[i].apiName,
                value: ele.value
              });
            }
            // else{
