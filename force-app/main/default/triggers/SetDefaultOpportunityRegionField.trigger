...
trigger SetDefaultOpportunityRegionField on Opportunity (before insert, before update) {
    ...
    for(Opportunity Op: Trigger.new){            
        ...
        if(rt_map.get(op.recordTypeID).getName().containsIgnoreCase('PGi') && (trigger.isInsert || (trigger.isUpdate && Op.OwnerId != trigger.oldMap.get(Op.Id).OwnerId))){
    ...
    if(OptyswithOwnerId.Size() > 0){
      ...
      //  system.debug('---------isInActiveTrigger----------------'+isInActiveTrigger);
        ...
        // query user region field
            map<Id,User> UserInfo  = new map<Id,User>([Select Id, User_Region__c,Manager.Name,Location__c,Channel__c From User where 
                ...
                for(String Uid : OptyswithOwnerId.keyset()){
                    if(UserInfo.containsKey(Uid)){
                        for(Opportunity Op : OptyswithOwnerId.get(Uid)){
                            Op.Owner_Region__c = UserInfo.get(Uid).User_Region__c; 
